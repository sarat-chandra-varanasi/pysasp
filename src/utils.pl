...
preamble(Head,L1,Lout) :- 
       ...
       (optimize(true) -> append(L2, [newline, tab, "if str(ctx) in nogood or is_inconsistent(atom(", Argsstring, "),ctx):", newline, tab(2), "return {'success': False, 'context': ctx}"], Lout) ; Lout = L2).
